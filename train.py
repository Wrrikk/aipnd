...
def build_model(arch, hidden_units):
    if arch == "vgg13":
        ...
        raise ValueError(
            f"Architecture '{arch}' not recognized. Please choose 'vgg13' or 'densenet121'."
        )
...
def train_model(model, trainloader, validloader, learning_rate, epochs, gpu):
    ...
    for epoch in range(epochs):
        ...
        # Improvement: Moved out of the 'else' block
        valid_loss = 0
        accuracy = 0
        model.eval()
        with torch.no_grad():
...
def main():
    ...
    train_model(
        model, trainloader, validloader, args.learning_rate, args.epochs, args.gpu
    )
